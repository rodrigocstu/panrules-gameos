...
`localStorage`, **aislada por navegador/dispositivo**. Hay cuatro claves relevantes:
...
| `panrules-gameos:cohorts:v1` | `src/hooks/useCohorts.js` | array de cohorts | **El único no acotado** (crece por cohort creado). |
...
- `byReason`: una clave por `reasonCode` distinto que emite el motor de validación. El conjunto de
  reason codes es cerrado y pequeño (decenas como máximo); cada entrada es `reasonCode → int`.
...
Si se cumple cualquiera de los tres, el beneficio de IndexedDB (cuota mayor + índices + escritura
